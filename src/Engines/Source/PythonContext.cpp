...
namespace Zeri::Engines::Defaults {
    ...
    ExecutionOutcome PythonContext::HandleCommand(
        ...
        if (cmd.commandName == "delete") {
            ...
            if (cmd.args[0] == "--hard") {
                if (cmd.args.size() < 4 || cmd.args[2] != "--confirm" || cmd.args[1] != cmd.args[3]) {
                    return std::unexpected(ExecutionError{
                        "PYTHON_DELETE_HARD_USAGE",
                        "Invalid hard delete syntax.",
                        cmd.rawInput,
                        { "Usage: /delete --hard <name> --confirm <name>" }
                    });
                }
                if (!HardDeleteScript(state, "python", cmd.args[1])) {
                    return std::unexpected(ExecutionError{ "PYTHON_SCRIPT_NOT_FOUND", "Script not found: " + cmd.args[1], cmd.rawInput });
                }
                return "Hard-deleted Python script: " + cmd.args[1];
            }
        ...
        if (cmd.commandName == "history") {
            ...
            const auto history = GetScriptHistory(state, "python", cmd.args[0]);
            if (history.empty()) {
                return std::unexpected(ExecutionError{ "PYTHON_HISTORY_NOT_FOUND", "No version history found for script: " + cmd.args[0], cmd.rawInput });
            }
        ...
        if (cmd.commandName == "diff") {
            ...
            const auto contentA = LoadScriptVersion(state, "python", cmd.args[0], versionA);
            const auto contentB = LoadScriptVersion(state, "python", cmd.args[0], versionB);
            if (!contentA.has_value() || !contentB.has_value()) {
                return std::unexpected(ExecutionError{
                    "PYTHON_DIFF_VERSION_NOT_FOUND",
        ...
        if (cmd.commandName == "rollback") {
            ...
            try {
                ...
                return std::unexpected(ExecutionError{
                    "PYTHON_ROLLBACK_VERSION_INVALID",
            ...
            if (!RollbackScript(state, "python", cmd.args[0], sourceVersion)) {
                return std::unexpected(ExecutionError{
                    "PYTHON_ROLLBACK_VERSION_NOT_FOUND",
                    "Version not found for rollback.",
                    cmd.rawInput
                });
            }
            return "Rollback completed for Python script: " + cmd.args[0];
        }
