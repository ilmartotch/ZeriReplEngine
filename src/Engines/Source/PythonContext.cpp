    ...
    ExecutionOutcome PythonContext::HandleCommand(
        ...
        if (cmd.commandName == "delete") {
            ...
            if (hardIt != cmd.flags.end()) {
                ...
                if (!HardDeleteScript(state, "python", hardTarget)) {
                    return std::unexpected(ExecutionError{ "PYTHON_SCRIPT_NOT_FOUND", "Script not found: " + hardTarget, cmd.rawInput });
                }
                return "Hard-deleted Python script: " + hardTarget;
            }
