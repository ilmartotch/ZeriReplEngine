    ...
    ExecutionOutcome JsContext::HandleCommand(
        ...
        if (cmd.commandName == "delete") {
            ...
            if (hardIt != cmd.flags.end()) {
                ...
                if (!HardDeleteScript(state, lang, hardTarget)) {
                    return std::unexpected(ExecutionError{ "JS_SCRIPT_NOT_FOUND", "Script not found: " + hardTarget, cmd.rawInput });
                }
                return "Hard-deleted " + display + " script: " + hardTarget;
            }
